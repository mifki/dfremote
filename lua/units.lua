...
function unit_fullprof(unit)
    ...
    if unit.enemy.undead then
        if #unit.enemy.undead.undead_name == 0 then
            prof = prof .. ' Corpse'
        else
            prof = dfhack.df2utf(unit.enemy.undead.undead_name):utf8capitalize() -- reanimated body parts use this string instead
        end
    end
    ...
    if not unit.job.current_job then
        ...
        if #unit.military.individual_drills > 0 then
            ...
            if s and s.cur_alert_idx ~= 0 then
                ...
                local activities = { sqpos.activities[0], sqpos.activities[1], sqpos.activities[2] }
                local events = { sqpos.events[0], sqpos.events[1], sqpos.events[2] }
...
function unit_get_thoughts(unitid, is_histfig)
    if istrue(is_histfig) then
        ...
        --[[if not unit.flags1.inactive and unit_iscitizen(unit) then
            local unitws = df.viewscreen_unitst:new()
            unitws.unit = unit
...
function unit_get_relationships(unitid)
    ...
    for i,v in ipairs(ws.relation_unit) do
        ...
        if v then
            ...
            id = v.id
            can_view = true
            can_zoom = not v.flags1.inactive
        else
...
function unit_get_assigned_animals(unitid)
    ...
    for i,unit in ipairs(df.global.world.units.active) do
        if unit.civ_id == df.global.ui.civ_id and unit.flags1.tame and not unit.flags1.inactive and not unit.flags1.forest then
